...
@pytest.mark.parametrize(
    "max_sentences, n_narrative, n_dialogue",
    [
        (2000, 20, 60),
        (50, 20, 25),
        (10, 5, 5),
...
def test_get_soda_dataset(mock_load_dataset, max_sentences, n_narrative, n_dialogue):
    dataset = get_soda_dataset(max_sentences=max_sentences)

    assert isinstance(dataset, dict)
    assert "narrative" in dataset
    assert "dialogue" in dataset

    # Dummy dataset has only 20 samples
    assert len(dataset["narrative"]) == n_narrative
    assert len(dataset["dialogue"]) == n_dialogue
